...
- [ ] 写表驱动失败测试：有下一阶段且支持 Direct LLM、无下一阶段、下一阶段仅手工提交、当前不能 Continue。
- [ ] 实现 `getContinueRunCapability`，不读取全局状态，不调用 IPC。
- [ ] 运行：
...
- [ ] 将当前页面中的 Continue 副作用移入 hook。
- [ ] 保持现有 optional/required/auto_if_valid 策略行为。

## Task 3：跨阶段草案交接

- [ ] Studio 保存 `PendingStageRunResult`。
- [ ] `StageRunPanel` 增加 `initialRunResult`，与手动 run 结果走同一草案审阅路径。
- [ ] 结果只注入匹配 stage 的面板。
- [ ] 消费或丢弃后清理 Studio 暂存状态。

## Task 4：双动作 UI

- [ ] `SceneStageFlowActions` 保留 Continue。
- [ ] 增加可选 `onContinueAndRun`、`canContinueAndRun`、`nextStageTitle`。
- [ ] 自动动作明确显示将运行下一阶段且仍需提交。
- [ ] running_next 时两个动作均禁用。
