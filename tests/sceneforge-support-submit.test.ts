...
describe('support stage submit', () => {
  ...
  it('submits reference reference_notes', async () => {
    const result = await service.submitStageDraft({
      projectDir: tmpDir,
      stage: 'reference',
      artifacts: [{ artifactKey: 'reference_notes', content: VALID_REFERENCE_NOTES }],
    });
  ...
  it('submits story story_direction', async () => {
    const result = await service.submitStageDraft({
      projectDir: tmpDir,
      stage: 'story',
      artifacts: [{ artifactKey: 'story_direction', content: VALID_STORY_DIRECTION }],
    });
    expect(result.validation.status).toBe('passed');
  });
      ...
      artifacts: [
        {
          ...
          content: VALID_PERFORMANCE_DIRECTION,
        },
      ],
    });
    expect(result.validation.status).toBe('passed');
      ...
      artifacts: [
        {
          ...
          content: `## voice_direction
### voice_identity_lock
旁白保持克制讲述感，主角维持压低声线。
### breath_pause_pattern
旁白在转场前短停顿半拍，主角关键句前先吸气。
### speaker_voice_notes
- 旁白：平稳、冷静、不过度煽情。
- 主角：中低频、收尾压低。
...
video_prompts 需继承人声节奏与 BGM / Foley-SFX / Ambience / Silence。
...
进入 video_prompts。`,
  ...
  it('fails script semantic validation for shell content', async () => {
    const result = await service.submitStageDraft({
      projectDir: tmpDir,
      stage: 'script',
    ...
    expect(result.validation.errors.some((error) => error.code === 'SCRIPT_MISSING_SEGMENTS')).toBe(true);
  });
